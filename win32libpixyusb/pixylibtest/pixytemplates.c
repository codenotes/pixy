 ...
 *	C:\lib\pixy\win32libpixyusb\pixylibtest\pixytemplates.tm
...
#endif
{
	...
	res = WSAbort ?
		WSPutFunction( mlp, "Abort", 0) : WSPutReal32( mlp, _rp0);
...
#endif
{
	int _res;
	_res = WSConnect(mlp);
	if (_res) _res = _definepattern(mlp, (char *)"test[]", (char *)"{}", 0);
	if (_res) _res = _definepattern(mlp, (char *)"getDistance[]", (char *)"{}", 1);
